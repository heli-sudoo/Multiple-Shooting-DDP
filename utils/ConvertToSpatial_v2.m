...
for i = 1:RBDtree.NumBodies
    if ~( contains(RBDtree.Bodies{i}.Joint.Type, 'fixed') || contains(RBDtree.Bodies{i}.Joint.Type, 'base'))
        nb = nb+1;
    end
end
...
for i = 1:RBDtree.NumBodies       
    ...
    if ~( contains(RBDtree.Bodies{i}.Joint.Type, 'fixed') || contains(RBDtree.Bodies{i}.Joint.Type, 'base'))
        ...
        inertia = GetInertiaMatrix(RBDtree.Bodies{i}.Inertia) + mass * skew(CoM)*skew(CoM);
